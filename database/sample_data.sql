...
-- Insert sample memos
-- Note: 実際のSupabase環境では、まずユーザーを作成してからこのクエリを実行してください
-- または、一時的に外部キー制約を無効にして実行することもできます

-- 方法1: 実際のユーザーIDを使用する場合（推奨）
-- 以下のクエリを実行する前に、SupabaseのAuthenticationでユーザーを作成し、
-- そのユーザーIDを取得して置き換えてください

-- 方法2: 一時的に外部キー制約を無効にしてサンプルデータを挿入
-- BEGIN;
